...
module.exports = function (config, errors, UserRepository, Movie, MovieUserAction) {
    ...
    function MovieRepository(user) {
        checkUserExistance(user);
        this.user = user;
    }

    MovieRepository.prototype.movieUserActionToResponse = function (movieUserAction) {
        var self = this;
        var ownWatched = {value: false};
        var userWatched = movieUserAction.watched.reduce(function (userWatched, watched) {
            if (watched.user._id.equals(self.user._id)) {
        ...
        var userRatings = movieUserAction.ratings.reduce(function (userRatings, rating) {
            ...
            if (rating.user._id.equals(self.user._id)) {
                ownRating = {};
    ...
    MovieRepository.prototype.create = function (movieData) {
        var self = this;
        var deferred = q.defer();
        movieData.lastModifiedUser = self.user._id;
        ...
        movie.save().then(function (movie) {
            ...
            return new MovieUserAction(newValue).save().then(function (result) {
                return self.movieUserActionToResponse(newValue);
            });
    ...
    MovieRepository.prototype.getAll = function (options) {
        var self = this;
        var deferred = q.defer();
        options = options || {};
        ...
        MovieUserAction.paginate(options).then(function (result) {
            var docs = result.docs.reduce(function (docs, movieUserAction) {
                if (movieUserAction.movie) {
                    docs.push(self.movieUserActionToResponse(movieUserAction));
                }
                return docs;
            }, []);
    ...
    MovieRepository.prototype.getById = function (id) {
        var self = this;
        var deferred = q.defer();
        var populate = {path: 'movie watched ratings'};
        findMovieById(id).then(function (result) {
            return deferred.resolve(self.movieUserActionToResponse(result));
        }).catch(function (error) {
            return deferred.reject(errors.convertError(error));
        });
        return deferred.promise;
    };
    MovieRepository.prototype.updateById = function (id, data) {
        var self = this;
        var deferred = q.defer();
        delete data._id;
        data.lastModifiedUser = self.user._id;
    ...
    MovieRepository.prototype.deleteById = function (id) {
        var self = this;
        var deferred = q.defer();
        findMovieById(id).then(function (result) {
            return MovieUserAction.remove().exec().then(function () {
                return self.movieUserActionToResponse(result);
            });
        }).then(function (result) {
    ...
    MovieRepository.prototype.setWatchedById = function (id) {
        var self = this;
        var deferred = q.defer();
        q.all([
            findMovieById(id),
            findWatchedById(id, self.user._id)
        ]).then(function (result) {
            var movie = result[0];
            ...
            movie.watched.addToSet({user: self.user});
            return movie.save();
        }).then(function (result) {
            deferred.resolve(self.movieUserActionToResponse(result));
        }).catch(function (error) {
            return deferred.reject(errors.convertError(error));
        });
        return deferred.promise;
    };
    MovieRepository.prototype.deleteWatchedById = function (id) {
        var self = this;
        var deferred = q.defer();
        q.all([
            findMovieById(id),
    ...
    MovieRepository.prototype.setRatingById = function (id, value) {
        var self = this;
        var deferred = q.defer();
        q.all([
            findMovieById(id),
            findRatingsById(id, self.user._id)
        ]).then(function (result) {
            var movie = result[0];
            var rating = result[1];
            if (rating) {
            ...
            result.ratings.addToSet({user: self.user, value: value});
            return result.save();
        }).then(function (result) {
            deferred.resolve(self.movieUserActionToResponse(result));
        }).catch(function (error) {
    ...
    MovieRepository.prototype.deleteRatingById = function (id) {
        var self = this;
        var deferred = q.defer();
        q.all([
            findMovieById(id),
